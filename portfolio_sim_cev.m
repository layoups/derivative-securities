...
%% call options
[CEV_call_returns, CEV_call_prices, CEV_call_strikes] = ... 
    all_eu(CEV_stocks, num_options, @price_call, @call_returns, ...
    rate, T, nTrials);

%% put options
[CEV_put_returns, CEV_put_prices, CEV_put_strikes] = ... 
    all_eu(CEV_stocks, num_options, @price_put, @put_returns, ...
    rate, T, nTrials);

%% Amp options
[CEV_amp_returns, CEV_amp_prices] = all_lookbacks(CEV_stocks, ...
    rate, T, @price_lookback, @amplitude_payoff);

%% final max options
[CEV_max_returns, CEV_max_prices] = all_lookbacks(CEV_stocks, ...
    rate, T, @price_lookback, @final_max_payoff);

%% min final options
[CEV_min_returns, CEV_min_prices] = all_lookbacks(CEV_stocks, ...
