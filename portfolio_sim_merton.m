...
aapl_merton_returns = mean(stock_returns(aapl_merton_sim));
jpm_merton_returns = mean(stock_returns(jpm_merton_sim));
pfe_merton_returns = mean(stock_returns(pfe_merton_sim));
tsla_merton_returns = mean(stock_returns(tsla_merton_sim));
...
%% call options
aapl_min_strike = round(aapl_merton_sim(end) * (0.80));
aapl_max_strike = round(aapl_merton_sim(end) * (1.20));
aapl_strike_step = (aapl_max_strike - aapl_min_strike) / 7;
[aapl_merton_call_price, aapl_strikes] = price_call(aapl_merton_sim, aapl_strike_step, aapl_min_strike, aapl_max_strike, nTrials, rate, T);
aapl_merton_call_returns = call_returns(aapl_merton_sim, aapl_strike_step, aapl_min_strike, aapl_max_strike, nTrials)'; 

jpm_min_strike = round(jpm_merton_sim(end) * (0.80));
jpm_max_strike = round(jpm_merton_sim(end) * (1.20));
jpm_strike_step = (jpm_max_strike - jpm_min_strike) / 7;
[jpm_merton_call_price, jpm_strikes] = price_call(jpm_merton_sim, jpm_strike_step, jpm_min_strike, jpm_max_strike, nTrials, rate, T);
jpm_merton_call_returns = call_returns(jpm_merton_sim, jpm_strike_step, jpm_min_strike, jpm_max_strike, nTrials)'; 

pfe_min_strike = round(pfe_merton_sim(end) * (0.80));
pfe_max_strike = round(pfe_merton_sim(end) * (1.20));
pfe_strike_step = (pfe_max_strike - pfe_min_strike) / 7;
[pfe_merton_call_price, pfe_strikes] = price_call(pfe_merton_sim, pfe_strike_step, pfe_min_strike, pfe_max_strike, nTrials, rate, T);
pfe_merton_call_returns = call_returns(pfe_merton_sim, pfe_strike_step, pfe_min_strike, pfe_max_strike, nTrials)'; 

tsla_min_strike = round(tsla_merton_sim(end) * (0.80));
tsla_max_strike = round(tsla_merton_sim(end) * (1.20));
tsla_strike_step = (tsla_max_strike - tsla_min_strike) / 7;
[tsla_merton_call_price, tsla_strikes] = price_call(tsla_merton_sim, tsla_strike_step, tsla_min_strike, tsla_max_strike, nTrials, rate, T);
tsla_merton_call_returns = call_returns(tsla_merton_sim, tsla_strike_step, tsla_min_strike, tsla_max_strike, nTrials)'; 

cvx_min_strike = round(cvx_merton_sim(end) * (0.80));
cvx_max_strike = round(cvx_merton_sim(end) * (1.20));
cvx_strike_step = (cvx_max_strike - cvx_min_strike) / 7;
[cvx_merton_call_price, cvx_strikes] = price_call(cvx_merton_sim, cvx_strike_step, cvx_min_strike, cvx_max_strike, nTrials, rate, T);
cvx_merton_call_returns = call_returns(cvx_merton_sim, cvx_strike_step, cvx_min_strike, cvx_max_strike, nTrials)'; 

dal_min_strike = round(dal_merton_sim(end) * (0.80));
dal_max_strike = round(dal_merton_sim(end) * (1.20));
...
aapl_max_strike = round(aapl_merton_sim(end) * (1.20));
aapl_strike_step = (aapl_max_strike - aapl_min_strike) / 7;
[aapl_merton_put_price, aapl_strikes] = price_put(aapl_merton_sim, aapl_strike_step, aapl_min_strike, aapl_max_strike, nTrials, rate, T);
aapl_merton_put_returns = put_returns(aapl_merton_sim, aapl_strike_step, aapl_min_strike, aapl_max_strike, nTrials)'; 

jpm_min_strike = round(jpm_merton_sim(end) * (0.80));
jpm_max_strike = round(jpm_merton_sim(end) * (1.20));
jpm_strike_step = (jpm_max_strike - jpm_min_strike) / 7;
[jpm_merton_put_price, jpm_strikes] = price_put(jpm_merton_sim, jpm_strike_step, jpm_min_strike, jpm_max_strike, nTrials, rate, T);
jpm_merton_put_returns = put_returns(jpm_merton_sim, jpm_strike_step, jpm_min_strike, jpm_max_strike, nTrials)'; 

pfe_min_strike = round(pfe_merton_sim(end) * (0.80));
pfe_max_strike = round(pfe_merton_sim(end) * (1.20));
pfe_strike_step = (pfe_max_strike - pfe_min_strike) / 7;
[pfe_merton_put_price, pfe_strikes] = price_put(pfe_merton_sim, pfe_strike_step, pfe_min_strike, pfe_max_strike, nTrials, rate, T);
pfe_merton_put_returns = put_returns(pfe_merton_sim, pfe_strike_step, pfe_min_strike, pfe_max_strike, nTrials)'; 

tsla_min_strike = round(tsla_merton_sim(end) * (0.80));
tsla_max_strike = round(tsla_merton_sim(end) * (1.20));
tsla_strike_step = (tsla_max_strike - tsla_min_strike) / 7;
[tsla_merton_put_price, tsla_strikes] = price_put(tsla_merton_sim, tsla_strike_step, tsla_min_strike, tsla_max_strike, nTrials, rate, T);
tsla_merton_put_returns = put_returns(tsla_merton_sim, tsla_strike_step, tsla_min_strike, tsla_max_strike, nTrials)'; 

cvx_min_strike = round(cvx_merton_sim(end) * (0.80));
cvx_max_strike = round(cvx_merton_sim(end) * (1.20));
cvx_strike_step = (cvx_max_strike - cvx_min_strike) / 7;
[cvx_merton_put_price, cvx_strikes] = price_put(cvx_merton_sim, cvx_strike_step, cvx_min_strike, cvx_max_strike, nTrials, rate, T);
cvx_merton_put_returns = put_returns(cvx_merton_sim, cvx_strike_step, cvx_min_strike, cvx_max_strike, nTrials)'; 
...
aapl_merton_amp_returns = mean(amplitude_payoff(aapl_merton_sim));

jpm_merton_amp_price = price_lookback(jpm_merton_sim, rate, T, @amplitude_payoff);
jpm_merton_amp_returns = mean(amplitude_payoff(jpm_merton_sim));

pfe_merton_amp_price = price_lookback(pfe_merton_sim, rate, T, @amplitude_payoff);
pfe_merton_amp_returns = mean(amplitude_payoff(pfe_merton_sim));

tsla_merton_amp_price = price_lookback(tsla_merton_sim, rate, T, @amplitude_payoff);
tsla_merton_amp_returns = mean(amplitude_payoff(tsla_merton_sim));

cvx_merton_amp_price = price_lookback(cvx_merton_sim, rate, T, @amplitude_payoff);
cvx_merton_amp_returns = mean(amplitude_payoff(cvx_merton_sim));
...
aapl_merton_min_returns = mean(min_final_payoff(aapl_merton_sim));

jpm_merton_min_price = price_lookback(jpm_merton_sim, rate, T, @min_final_payoff);
jpm_merton_min_returns = mean(min_final_payoff(jpm_merton_sim));

pfe_merton_min_price = price_lookback(pfe_merton_sim, rate, T, @min_final_payoff);
pfe_merton_min_returns = mean(min_final_payoff(pfe_merton_sim));

tsla_merton_min_price = price_lookback(tsla_merton_sim, rate, T, @min_final_payoff);
tsla_merton_min_returns = mean(min_final_payoff(tsla_merton_sim));

cvx_merton_min_price = price_lookback(cvx_merton_sim, rate, T, @min_final_payoff);
cvx_merton_min_returns = mean(min_final_payoff(cvx_merton_sim));
